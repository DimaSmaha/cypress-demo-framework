...
describe("4 Commands Probably You Did NOT Know", () => {
  it("isVisible DEMO", () => {
    ...
    cy.get(".collapse")
      ...
      .then(($element) => {
        cy.log(
          `Collapse content as soon as the website is loaded:  ${Cypress.dom.isVisible(
            $element
          )}`
        );
      });
    ...
    cy.get(".collapse")
      ...
      .then(($element) => {
        cy.log(
          `Collapse content as soon as I click on the card:  ${Cypress.dom.isVisible(
            $element
          )}`
        );
      });

    cy.get('[class="main-header"]').then(($el) => {
  ...
  it("isVisible DEMO", () => {
    ...
    cy.get('[id="userEmail"]').then(($el) => {
      cy.log(Cypress.dom.getInputFromLabel($el));
    });
  });
});
